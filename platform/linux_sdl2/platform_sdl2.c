...
static unsigned int _sdl_get_button_state() {
    ...
    // 您可以根据喜好更改这些按键映射
    if (keyboard_state[SDL_SCANCODE_LEFT] || keyboard_state[SDL_SCANCODE_A])  state |= buttonLeft;
    if (keyboard_state[SDL_SCANCODE_RIGHT] || keyboard_state[SDL_SCANCODE_D]) state |= buttonRight;
    if (keyboard_state[SDL_SCANCODE_UP] || keyboard_state[SDL_SCANCODE_W])    state |= buttonUp;
    if (keyboard_state[SDL_SCANCODE_DOWN] || keyboard_state[SDL_SCANCODE_S])  state |= buttonDown;
    if (keyboard_state[SDL_SCANCODE_Z] || keyboard_state[SDL_SCANCODE_J])     state |= buttonA;
    if (keyboard_state[SDL_SCANCODE_X] || keyboard_state[SDL_SCANCODE_K])     state |= buttonB;
...
static platform_image_t _sdl_gfx_create_image(int width, int height, const void* data) {
    SDL_Texture* texture = SDL_CreateTexture(g_renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STATIC, width, height);
    if (!texture) {
        SDL_Log("Failed to create texture: %s", SDL_GetError());
        return NULL;
    }
    if (data) {
        SDL_UpdateTexture(texture, NULL, data, width * 4); // 4 bytes per pixel (RGBA)
    }
    // 设置默认混合模式为透明
...
static platform_image_t _sdl_gfx_create_render_target(int width, int height) {
    SDL_Texture* texture = SDL_CreateTexture(g_renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, width, height);
    if (!texture) {
...
static void _sdl_gfx_draw_image(platform_image_t image, const Rect* src_rect, int x, int y, ImageFlip flip) {
    ...
    if (flip == imageFlippedX)  sdl_flip = SDL_FLIP_HORIZONTAL;
    if (flip == imageFlippedY)  sdl_flip = SDL_FLIP_VERTICAL;
    if (flip == imageFlippedXY) sdl_flip = (SDL_RendererFlip)(SDL_FLIP_HORIZONTAL | SDL_FLIP_VERTICAL);
...
int platform_sdl2_init(const char* title, int width, int height, int target_fps) {
    ...
    g_window = SDL_CreateWindow(title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, SDL_WINDOW_SHOWN);
    if (g_window == NULL) {
    ...
    // 填充API结构体
    platform_api_t sdl_api = {
        .mem_alloc = _sdl_mem_alloc,
